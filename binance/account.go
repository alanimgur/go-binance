...
func (b *Binance) PlaceLimitOrder(l LimitOrder) (res PlacedOrder, err error) {
	reqURL := fmt.Sprintf("api/v3/order?symbol=%s&side=%s&type=%s&quantity=%.8f&price=%.8f&recvWindow=%d", l.Symbol, l.Side, l.Type, l.Quantity, l.Price, l.RecvWindow)

	if l.Type == "LIMIT" {
		reqURL = fmt.Sprintf("%s&timeInForce=%s", reqURL, l.TimeInForce)
	}
...
func (b *Binance) PlaceMarketOrder(m MarketOrder) (res PlacedOrder, err error) {
	...
	reqURL := fmt.Sprintf("api/v3/order?symbol=%s&side=%s&type=%s&quantity=%.8f&recvWindow=%d", m.Symbol, m.Side, m.Type, m.Quantity, m.RecvWindow)
